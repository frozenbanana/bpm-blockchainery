...
Blockchain \cite{nakamoto2008bitcoin}, invented by Saitoshi Nakamoto as a mean to solve the problem of double-spending in crypto-currencies, is a chain of digitally signed data (so called blocks). To sign a block, the signature (hash) of a block is calculated by applying a hash-function to the data the block represents and the hash of the previous block. Because the nodes in the network have to "agree" on which block to include next, there has to a consensus-mechanism. In the case of the Bitcoin network this consensus is reached by making the addition of a block computationally hard (proof of work). This attribute also makes the data represented in a block difficult to change, once the block has been added to the network and especially after the block in question was succeeded by other blocks. Since blockchain networks provide a consensus mechanism and are tamper-proof, they do not require a centralized authority. 

These capabilities (storing data immutably and having consent on a single state without the need of a  central authority) are greatly increased by networks like Ethereum. They provide developers with the ability to write Turing complete programs, which are then executed on the chain \cite{buterin2014next}. These programs are called smart contracts. 

Because smart contracts are blockchain-based and are executed within the blockchain-network, they are ideal for business process execution, since the entities collaborating with each other now have a single source of truth, when it comes to the state of the business process they are executing or the messages that were exchanged. Also since it is impossible (or at least very hard) to retroactively change already submitted states, solutions that are based on smart contracts are ideal to serve as audit trails.

As shown in \cite{weber2016untrusted} there are ways to extend the use cases of business process execution and auditing to an untrusted environment, thus making it possible for entities to collaborate with each other that would usually not be able or willing to do so. The execution of untrusted business processes is made possible by using active choreographies, which we will briefly discuss in section \ref{sec:rel_work}.

The goal of this paper is to go even further than \cite{weber2016untrusted}: We aim to not only make it possible to execute business processes in an untrusted environment, but to enforce a business process without reveiling its structure or purpose. Our approach also aims to keep secret what was communicated during the process execution and who was communicating, while still making sure that the predefined procedure is being followed by all participants and that an immutable audit trail is recorded that is visible only by entities that have a right to see it. 
